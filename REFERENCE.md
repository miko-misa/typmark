...
The renderer adapts low-chroma SVG paints to its light or dark theme in OKLab instead of inverting every color. Neutral foregrounds and backgrounds follow the browser CSS colors in `--typmark-svg-fg` and `--typmark-svg-bg`, while chromatic paints keep their hue. In `auto` mode the mapping follows `prefers-color-scheme`. The renderer script also writes trusted element-local paint declarations so ordinary page CSS cannot accidentally recolor the embedded diagram; the stylesheet provides a black/white fallback for script-free and PDF output. User and browser forced-color rules are not disabled. Typst SVG fragments inserted later are detected automatically. Raw core/WASM fragments must be paired with the renderer assets to receive this behavior; the WASM API returns raw HTML and therefore requires trusted input or host-side sanitization.
